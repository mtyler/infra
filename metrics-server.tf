...
## 
##   curl https://raw.githubusercontent.com/techiescamp/kubeadm-scripts/main/manifests/metrics-server.yaml -o ./metrics-server.yaml
##
##   k2tf -f metrics-server.yaml -o metrics-server.terraform 
##
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["/metrics-server", "--cert-dir=/tmp", "--secure-port=10250", "--kubelet-preferred-address-types=InternalIP,ExternalIP,Hostname", "--kubelet-use-node-status-port", "--metric-resolution=15s", "--kubelet-insecure-tls"]
